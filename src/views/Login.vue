<template>
  ...
  >
    ...
    <a-typography>
      ...
      <a-typography-paragraph
        >1.1
        Ginkgo网的运营者及相关关联公司（以下简称“Ginkgo”）根据本使用协议的条款及不时发布的规则为你提供基于Ginkgo网（包括Ginkgo
        pc端、Ginkgo及Ginkgo相关客户端、移动网页端等）的互联网服务。本协议的条款可由Ginkgo随时修改，修改后的使用协议在网站上一经公布即有效代替原来的使用协议。你一旦在Ginkgo网注册，即成为Ginkgo用户（以下简称“用户”或“你”），并受本协议的约束。
      </a-typography-paragraph>
      <a-typography-paragraph
        >1.2
      ...
      <a-typography-paragraph
        >2.1
        Ginkgo向用户提供包括但不限于如下服务：发布并分享对NFT数字艺术品、收藏品等的评论；NFT数字艺术品、收藏品；在Ginkgo中发布话题、日记、上传图片、视频等；在论坛中发布或参加各类活动；电子商务服务；付费内容服务等。除非本协议另有其它明示规定，Ginkgo增加或强化目前本服务的任何新功能，包括所推出的新产品，均受本使用协议之规范。
      </a-typography-paragraph>
      <a-typography-paragraph
        >2.2
      ...
      <a-typography-title :level="4"
        >4、用户账号、密码及安全</a-typography-title
      >
      <a-typography-paragraph
        >4.1
        完成本服务的注册程序并成功注册之后，你可使用你的注册邮箱/手机号码和密码，登录到你在Ginkgo网的账号（下称“账号”）。注册完成之时，你便获得了账号的使用权。保护账号安全，是你的责任。
      </a-typography-paragraph>
      <a-typography-paragraph
        >4.2 你应对所有使用你的账号的行为负完全的责任。你同意：
        1）你的Ginkgo账号遭到未获授权的使用，或者发生其它任何安全问题时，你将立即通知Ginkgo；
        2）如果你未保管好自己的账号和密码，因此而产生的任何损失或损害，Ginkgo不承担任何责任；
        3）你要对使用账号的所有行为给你、Ginkgo或第三方造成的损害负全责，包括你未保管好自己的账号或密码的情形。
      </a-typography-paragraph>
      <a-typography-paragraph
        >4.3
        由于通过账号可获取到用户的个人信息，且账号的所有权归Ginkgo所有，除法律明文规定外，未经Ginkgo同意，用户不得将账号转让、出售或出借给他人使用
      </a-typography-paragraph>
    </a-typography>
  </a-drawer>
</template>
